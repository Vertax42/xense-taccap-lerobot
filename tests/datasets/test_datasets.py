...
@pytest.mark.skipif(
    not (Path("tests/artifacts/datasets") / "lerobot" / "pusht" / "frame_0.safetensors").exists(),
    # Only the generator script is committed, not the golden frames it writes,
    # so a fresh clone cannot run this. (It also downloads the dataset from the
    # Hub, which a sandboxed CI run cannot do either.)
    reason="dataset backward-compatibility artifacts absent (see tests/artifacts/datasets/)",
)
